...
export default function SearchBar({ nodes = [], onSelectNode, onSelectColumn }) {
    ...
    <div ref={containerRef} className="relative w-72 md:w-80 lg:w-96">
      ...
      <div className="relative flex items-center">
        ...
        <input
          ...
          className="w-full bg-slate-950/80 border border-slate-800 hover:border-slate-700 focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 rounded-lg pl-9 pr-8 py-1.5 text-xs text-slate-100 placeholder-slate-500 transition-all outline-none"
        />
        {query ? (
          <button
            onClick={() => {
          ...
          <kbd className="absolute right-2 text-[10px] text-slate-500 font-mono bg-slate-900 border border-slate-800 px-1.5 py-0.5 rounded pointer-events-none hidden sm:inline-block">
            ⌘K
          </kbd>
        )}
      </div>
      ...
      {isOpen && query.trim().length > 0 && (
        <div className="absolute top-full mt-2 left-0 right-0 bg-slate-900/95 backdrop-blur-xl border border-slate-800 rounded-xl shadow-2xl overflow-hidden z-50 max-h-[420px] overflow-y-auto divide-y divide-slate-800/60 animate-in fade-in zoom-in-95 duration-150">
          {totalResults === 0 ? (
            ...
            <>
                ...
                <div className="p-2">
                  ...
                  {results.reports.map((r) => (
                    <div
                      ...
                      className="flex items-center justify-between px-2.5 py-2 rounded-lg text-xs hover:bg-slate-800/80 cursor-pointer transition-colors group"
                    >
                      <div className="flex items-center gap-2 overflow-hidden">
                        ...
                        <div className="overflow-hidden">
                          <p className="font-semibold text-slate-100 group-hover:text-emerald-300 transition-colors truncate">
                            {r.name}
                          </p>
                      ...
                      <span className="text-[10px] text-emerald-400 bg-emerald-500/10 border border-emerald-500/30 px-1.5 py-0.5 rounded font-medium shrink-0">
                        Focus Report
                      </span>
                    </div>
                  ))}
              ...
              {results.tables.length > 0 && (
                <div className="p-2">
                  ...
                  {results.tables.map((t) => (
                    ...
                    >
                      <div className="flex items-center gap-2 overflow-hidden">
                        <div className="p-1 rounded bg-indigo-500/10 text-indigo-400 border border-indigo-500/20 shrink-0">
                          {t.type === 'source_table' ? <Database className="w-3.5 h-3.5 text-amber-400" /> : <Table className="w-3.5 h-3.5" />}
                        </div>
                        <div className="overflow-hidden">
              ...
              {results.columns.length > 0 && (
                <div className="p-2">
                  <div className="px-2 py-1 text-[10px] font-bold uppercase tracking-wider text-cyan-400 flex items-center justify-between">
                  ...
                  {results.columns.map((c, i) => (
                    ...
                    >
                      <div className="flex items-center gap-2 overflow-hidden">
                        ...
                        <div className="overflow-hidden">
                          <p className="font-semibold text-slate-100 group-hover:text-cyan-300 transition-colors truncate">
                            {c.column.name}
                          </p>
                          <p className="text-[10px] text-slate-400 truncate">
                            in <span className="text-slate-300 font-medium">{c.nodeName}</span> ({c.nodeContainer})
                          </p>
                        </div>
                      </div>
                      <span className="text-[10px] text-indigo-400 bg-indigo-500/10 border border-indigo-500/30 px-1.5 py-0.5 rounded font-medium shrink-0 flex items-center gap-1">
